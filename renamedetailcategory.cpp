...
void RenameDetailCategory::renameDetailCategorySlot()
{
    ...
    QDir tmpDir;
    QString newDetailCategory = detailCategoryInput->text().simplified();
    foreach(QString dirName, dir.entryList(QDir::Dirs | QDir::NoDotAndDotDot))
    {
        ...
        {
            bool sucessful = tmpDir.rename(carModelDir.path()+"/"+dirName2+"/"+oldName,carModelDir.path()+"/"+dirName2+"/"+newDetailCategory);
            qDebug()<<"Renaming "+ carModelDir.path()+"/"+dirName2+"/"+oldName+" into "+carModelDir.path()+"/"+dirName2+"/"+newDetailCategory+" is "+sucessful;
        }
    }
    QStringList* details = this->map->value(oldName);
    this->map->remove(oldName);
    this->map->insert(newDetailCategory,details);
    this->close();
