    ...
    ui(new Ui::MainWindow)
{
    ...
    this->history = new QList<QModelIndex>();
    initWindowSize();

    isUpdateDetailGalleryFinished = true;
    isUpdateGalleryFinished = true;

//    QHBoxLayout *menuBarAlignment = new QHBoxLayout;
    ...
    //this->menuBar->setMaximumWidth(120);
    ...
    //выбор в двух окнах одного и того же элемента
    QObject::connect(ui->gallery,SIGNAL(clicked(QModelIndex)),this,SLOT(selectFromGalleryToList(QModelIndex)));
    QObject::connect(ui->galleryFilesList, SIGNAL(clicked(QModelIndex)),this,SLOT(selectFromListToGallery(QModelIndex)));


    //открытие фотки по двойному щелчку
    QObject::connect(ui->gallery,SIGNAL(doubleClicked(QModelIndex)),this,SLOT(selectAndOpen(QModelIndex)));
    QObject::connect(ui->galleryFilesList, SIGNAL(doubleClicked(QModelIndex)),this,SLOT(selectAndOpen(QModelIndex)));
    ...
    this->servicePath = new QMenu("Путь к галлерее: " + galleryPath);
    this->menuBar->addMenu(servicePath);
    menuBar->actions().at(1)->setDisabled(true);
    ...
    ui->galleryFilesList->setIconSize(QSize(15, 15 ) );
...
//doesnt work
void MainWindow::selectFromGalleryToList(QModelIndex i){
    ui->galleryFilesList->setSelectionBehavior (QAbstractItemView::SelectRows);
    qDebug()<<i.row();
        ui->galleryFilesList->selectionModel()->select (
        QItemSelection (
            ui->galleryFilesList->model ()->index (3, 0),
            ui->galleryFilesList->model ()->index (3, ui->galleryFilesList->model ()->columnCount () - 1)),
        QItemSelectionModel::Select);

}

void MainWindow::selectFromListToGallery(QModelIndex i){
        ui->gallery->setCurrentRow(i.row(), QItemSelectionModel::Toggle);
        QScrollBar *vb = ui->gallery->verticalScrollBar();
        qDebug()<<vb->value();

}

void MainWindow::selectAndOpen(QModelIndex i){
    openImage(i);
}
...
void MainWindow::createCarMakeSlot()
{
    ...
    if(detailsMap->isEmpty())
        qDebug()<<"createCarMakeSlot";
    else
        qDebug()<<"no createCarMakeSlot";
    QList<QString> keys = detailsMap->keys();

    foreach(QString key , keys){
        qDebug()<<"KEY="+key;
        QStringList* v = detailsMap->value(key);
        for(int i=0; i<v->size(); ++i)
            qDebug()<<v->at(i).toLocal8Bit().constData();
        }
    ...
    QObject::connect(m, SIGNAL(selectRowSignal(QString)), this, SLOT(selectRowSlot(QString)));
}
void MainWindow::selectRowSlot(QString whatToBeSelected)
{
  this->sPath = whatToBeSelected;
  ui->carModel->setModel(fileModelCarModel);
  this->ui->carModel->setRootIndex(fileModelCarModel->setRootPath(sPath));
  clearArticleGallery();
  QModelIndex* tmpIndex = returnFirstChildIndex(sPath, fileModelCarModel);
...
void MainWindow::deleteArticleSlot()
{
    ...
    d->show();
    QFile ffile("articleExsistence.txt");
    if(ffile.exists()) {
        ffile.open(QIODevice::ReadWrite |  QFile::Text);
        QTextStream out(&ffile);
        QTextStream in(&ffile);
        QString article;
        while(!(article=out.readLine()).isNull()){
            if(!article.contains(this->ui->detailArticle->currentIndex().data().toString())){
                in<<article+"\r\n";
            }
        }
        ffile.resize(0);
...
{
    DeleteImageWindow *d = new DeleteImageWindow();
    d->show();
    QObject::connect(d,SIGNAL(updateGallery()),this,SLOT(deleteImageSlotSlot()));

}
void MainWindow::deleteImageSlotSlot(){
    ...
    if(this->ui->detailCategory->currentIndex().isValid()) {
        if(this->editDetail == false) {
            ...
            if(this->ui->articleOutput->text()!="") {
                QString articleName=this->ui->articleOutput->text();
                QDir dir(detailPath+"/"+articleName);
                //работает, но долго
//                QDirIterator it("D:/carShop", QStringList() << articleName, QDir::Dirs, QDirIterator::Subdirectories);
//                while (it.hasNext()){
//                    showErrorWindow("Такой артикул уже есть: " +it.next());
//                    return;
//                }

                QFile sizeFile("articleExsistence.txt");
                if(sizeFile.exists()) {
                    sizeFile.open(QIODevice::ReadWrite |  QFile::Text);
                    QTextStream out(&sizeFile);
                    QString article;
                    while((article=out.readLine())!=NULL)
                    if(article.contains(articleName)){
                        qDebug()<<article<<article.indexOf(" ");
                        showErrorWindow("Такой артикул уже есть: " + article.right(article.size()-article.indexOf(" ")));
                        return;
                    }
                    sizeFile.close();
                }
                if(!dir.exists())
                {
                     ...
                     QTextStream out(&sizeFile);
                     out<<articleName<<" "<<dir.absolutePath()<<endl;
                     sizeFile.close();
//                     if(sizeFile.exists()) {
...
//                         sizeFile.open(QIODevice::ReadWrite | QFile::Append | QFile::Text);
//                         QTextStream out(&sizeFile);
//                         out<<articleName<<" "<<dir.currentPath()<<endl;
//                     }
                } else {
                             ...
                             int imageSize)
{
    ...
    menuBar->actions().at(1)->setText(galleryPath);
...
void MainWindow::updateGallery()
{
    ...
    if(isUpdateGalleryFinished) {
        ...
        ui->galleryFilesList->setRootIndex(filemodel->setRootPath(currentGalleryPath));
        menuBar->actions().at(1)->setText(currentGalleryPath);
...
void MainWindow::updateGalleryFinishedSlot()
{
    ...
    isUpdateGalleryFinished = true;
    /*scroll to last selected folder*/
    if(!history->isEmpty()&&isBackPushed){
        QScrollBar *vb = ui->gallery->verticalScrollBar();
...
void MainWindow::on_backButton_clicked()
{
    ...
    updateGallery();
    QString prevFolder = currentGalleryPath.split("/").last();

    //ui->gallery->setCurrentRow(, QItemSelectionModel::Toggle);
}
